...
function die {
    ...
    echo -e "$prefix: "
    echo -e "$prefix: ${bold}You can contact mull-project for further assistance.${reset}"
    echo -e "$prefix: "
    
    echo -e "$prefix: ${bold}URL: https://github.com/mull-project/mull${reset}"
    echo -e "$prefix: "
...
function config_url {
    echo "https://dl.cloudsmith.io/public/mull-project/mull-stable/config.deb.txt?distro=${distro}&codename=${codename}&version=${version}&arch=${arch}" | sed 's/ /%20/g'
}
...
EOF
 ...
 >>>>: doesn't work after an hour, please contact mull-project
 >>>>: for assistance.
 >>>>:
EOF
        }
...
function import_gpg_key {
    local text="Importing 'mull-project/mull-stable' repository GPG key into apt ..."
    echo_running "$text"
    curl -1sLf "https://dl.cloudsmith.io/public/mull-project/mull-stable/gpg.41DB35380DE6BD6F.key" | apt-key add - &>$tmp_log
    echo_okfail "$text" || die "Could not import the GPG key for this repository"
}

function setup_repository {
    local repo_path="/etc/apt/sources.list.d/mull-project-mull-stable.list"
    check_fetch_config

    local text="Installing 'mull-project/mull-stable' repository via apt ..."
    echo_running "$text"
...
echo "Executing the  setup script for the 'mull-project/mull-stable' repository ..."
echo
